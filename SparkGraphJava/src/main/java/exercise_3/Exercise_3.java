...
public class Exercise_3 {
    ...
    private static class VProg extends AbstractFunction3<Long, Vertex, Vertex, Vertex> implements Serializable {
        ...
        public Vertex apply(Long vertexID, Vertex vertexValue, Vertex message) {
            ...
            System.out.println(vertexID.toString() + ',' + vertexValue.toString() + ',' + message.toString());
//            ›
    ...
    private static class sendMsg extends AbstractFunction1<EdgeTriplet<Vertex, Integer>, Iterator<Tuple2<Object, Vertex>>> implements Serializable {
        @Override
            ...
            if (sourceVertex._2.getValue() == Integer.MAX_VALUE ||  newDestVertex >= dstVertex._2.getValue()) {
                ...
                return JavaConverters.asScalaIteratorConverter(Arrays.asList(new Tuple2<Object, Vertex>(triplet.dstId(), newVertex)).iterator()).asScala();
            }
        }
    }
    ...
    public static void shortestPathsExt(JavaSparkContext ctx) {
        ...
        Graph<Vertex, Integer> G = Graph.apply(verticesRDD.rdd(), edgesRDD.rdd(), null, StorageLevel.MEMORY_ONLY(), StorageLevel.MEMORY_ONLY(),
