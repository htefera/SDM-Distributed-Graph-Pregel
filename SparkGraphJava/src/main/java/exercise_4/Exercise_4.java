...
public class Exercise_4 {
	...
	public static void wikipedia(JavaSparkContext ctx, SQLContext sqlCtx) throws FileNotFoundException {
		...
		List<Double> reset_probs = List.of(0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50);
//		List<Double> reset_probs = List.of(0.15);
		List<Integer> max_iters = IntStream.range(4, 21).filter(x -> x % 2 == 0).boxed().collect(Collectors.toList());
		...
		for (Double reset_prob: reset_probs){
			for (Integer max_iter: max_iters){
				...
				pagerank_graph.vertices().limit(10).show();
